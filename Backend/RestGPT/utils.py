...
class MyRotatingFileHandler(BaseRotatingHandler):
    ...
    def doRollover(self):
        ...
        dfn = self.rotation_filename(
            '.'.join(self.baseFilename.split('.')[:-1]) + f"_{self.cnt}." + self.baseFilename.split('.')[-1])
...
def init_spotify(requests_wrapper):
    ...
    artist_ids = \
    json.loads(requests_wrapper.get('https://api.spotify.com/v1/me/following?type=artist').text)['artists']['items']
    ...
    playlist_id_1 = requests_wrapper.post(f'https://api.spotify.com/v1/users/{user_id}/playlists',
                                          data={'name': 'My R&B'})
    ...
    playlist_id_2 = requests_wrapper.post(f'https://api.spotify.com/v1/users/{user_id}/playlists',
                                          data={'name': 'My Rock'})
