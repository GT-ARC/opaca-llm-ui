...
class LocalizationManager {
    ...
    _verifySettings() {
        if (!localizationData[this.language] || !localizationData[this.fallbackLanguage]) {
            throw Error(`Invalid languages configured in LocalizationManager: ${this.language}, ${this.fallbackLanguage}`);
        }
    }

    getText(key) {
        const text = conf.translations[this.selectedLanguage][key];
        const fallbackText = conf.translations[this.fallbackLanguage] || `<unknown text key: ${key}>`;
        return text ?? fallbackText;
    }

    getFormattedText(key, ...args) {
        return this.formatText(this.getText(key), ...args);
    }
